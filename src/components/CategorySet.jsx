      ...
      <div className='border shadow border-2 border-dark p-1'>
           {
               clist?.length > 0 ?
                   <div>
                       {clist?.map(item => (
                          <div className='border w-100 border-2 shadow p-2 mb-3'onDragOver={(e)=>handleDragOver(e)} onDrop={(e)=>handleDrop(e,item)}>
                            <div className='d-flex justify-content-between'>
                                <h3>{item.category}</h3>
                                <button className='btn'onClick={()=>{deleteCat(item.id)}}>
                                <i className="fa-solid fa-trash" style={{color: "#104975",}} />
                                </button>
                            </div>
